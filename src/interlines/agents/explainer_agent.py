...
def run_explainer_stub(
    ...
    *,
    ...
    """Compatibility wrapper around :func:`run_explainer` for the legacy pipeline.

    Behaviour
    ---------
    1. Call :func:`run_explainer` and, on success, return the list of
       :class:`ExplanationCard` instances.
    2. If the Result is an error whose string representation mentions
       a missing API key, build local stub cards from the parsed chunks
       and return those instead.
    3. For all other errors, raise :class:`RuntimeError`.

    Both the value stored under ``target_key`` on the blackboard and the
    return value of this function are always a ``list[ExplanationCard]`` so
    callers can rely on write-through semantics.
    """
    # Safely retrieve and normalise parser output for the offline fallback.
    ...
    # Primary path: try to run the real explainer (potentially LLM-backed).
    result: Result[list[ExplanationCard], str] = run_explainer(bb)

    if result.is_ok():
        # Happy path: propagate the cards returned by `run_explainer`.
        cards = result.unwrap([])  # default is only used on logically-impossible paths
        bb.put(target_key, cards)
        return cards

    # Error path: inspect the error and decide whether to fall back.
    error_msg = str(result)
    if "Missing API key" in error_msg:
        # Offline / CI-friendly fallback: build local stub cards from parser output.
